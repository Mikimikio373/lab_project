...
int cut_fit_FastReadCSV(int shift_X, int shift_Y)
{
  logon();
  ...
  string basepath = "GrainMatching_loop";
	...
	for (int vx = 0; vx < shift_X; vx++)
	{
		for (int vy = 0; vy < shift_Y; vy++)
		{
			...
			const char* filepath = filepath_s.c_str();
      ...
      fclose(fp); // ファイルを閉じる
			...
			char cut1[100];
			sprintf(cut1, "dx>%f && dx<%f && dy>%f && dy<%f && flag==0", left, right, bottom, top);
			cerr << cut1 << endl;
			TCut tcut1 = cut1;
			...
			t->Draw("dy:dx >> dxy_cut1", tcut1, "colz");
			c1->Print(fname);
			c1->Clear();
			...
			char cut2[100];
			sprintf(cut2, "dx>%f && dx<%f && dy>%f && dy<%f && flag==0", left, right, bottom, top);
			cerr << cut2 << endl;
			TCut tcut2 = cut2;
			...
			t->Draw("dy:dx >> dxy_cut2", tcut2, "colz");
			c1->Print(fname);
			c1->Clear();
			...
			char cut3[100];
			sprintf(cut3, "dx>%f && dx<%f && dy>%f && dy<%f && flag==0", left, right, bottom, top);
			cerr << cut3 << endl;
			TCut tcut3 = cut3;
			...
			t->Draw("dy:dx >> dxy_cut3", tcut2, "colz");
			...
			const char* rootfilepath = rootfilepath_s.c_str();
			TFile *fout = new TFile(rootfilepath, "recreate");
      t->Write();  
      fout->Close();
			delete t;
			delete dxy;
